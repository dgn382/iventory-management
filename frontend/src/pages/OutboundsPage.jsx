...
export default function OutboundsPage() {
  ...
  const handleBarcodeAdd = () => {
    ...
    const matchedItem = (selectedOrder.items || []).find((item) => String(item.product_barcode || item.barcode || '') === barcode);
    if (!matchedItem) {
      alert('Không tìm thấy sản phẩm trong đơn theo barcode này');
      return;
    }

    setSelectedOrder((prev) => ({
      ...prev,
      items: (prev.items || []).map((item) => String(item.id) === String(matchedItem.id)
