...
export default function ProductsPage() {
  ...
  const openAddModal = () => {
    setEditingId(null);
    setNewSku(''); setNewBarcode(''); setNewName(''); setNewPrice(''); setNewUnit('cái');
    setNewCategory(''); setNewImageUrl(''); setNewMinStock('50');
    setInitialStock(0); setTargetWarehouse('all');
    setIsModalOpen(true);
  };

  const openBarcodePreview = (product) => setBarcodePreviewProduct(product);
  const closeBarcodePreview = () => setBarcodePreviewProduct(null);
  const copyBarcode = async (barcode) => {
    if (!barcode) return;
    try {
      await navigator.clipboard.writeText(barcode);
      alert('Đã copy barcode');
    } catch {
      alert('Không thể copy barcode');
    }
  };
  ...
  const openEditModal = (product) => {
    ...
    setNewSku(product.sku || ''); setNewBarcode(product.barcode || ''); setNewName(product.name);
    ...
    try {
      if (editingId) {
        ...
        await api.put(`/products/${editingId}`, { 
          sku: newSku, barcode: newBarcode, name: newName, sale_price: newPrice, unit: newUnit,
      ...
      setNewSku(''); setNewBarcode(''); setNewName(''); setNewPrice(''); setNewUnit('cái');
      setNewCategory(''); setNewImageUrl(''); setNewMinStock('50'); setInitialStock(''); setBarcodeSearchText('');
      fetchData();
    } catch (error) { 
  ...
  const filteredProducts = products.filter((item) => {
    ...
    const barcodeSearch = barcodeSearchText.trim().toLowerCase();
    const matchesSearch = !search || [item.name, item.sku, item.category].some((value) => String(value || '').toLowerCase().includes(search));
    const matchesBarcode = !barcodeSearch || String(item.barcode || '').toLowerCase().includes(barcodeSearch);
    return matchesStatus && matchesSearch && matchesBarcode;
  });
  ...
  return (
    <div style={{ padding: '16px', background: '#f7fafc', minHeight: '100vh' }}>
                  ...
                  <div style={{ padding: '14px 14px 0' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px', gap: '10px' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                        ...
                        <span style={{ fontSize: '11px', color: '#64748b' }}>{item.barcode || 'Chưa có barcode'}</span>
                      </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap', marginBottom: '10px' }}>
                      <div style={{ display: 'inline-flex', padding: '5px 10px', borderRadius: '999px', background: '#eff6ff', color: '#2563eb', fontSize: '12px', fontWeight: '700' }}>
                        {item.category || 'Chưa có danh mục'}
                      </div>
                      <button type="button" onClick={() => copyBarcode(item.barcode)} disabled={!item.barcode} style={{ padding: '5px 10px', borderRadius: '999px', border: '1px solid #dbe3ee', background: item.barcode ? '#fff' : '#f8fafc', color: item.barcode ? '#0f172a' : '#94a3b8', fontSize: '12px', fontWeight: '700', cursor: item.barcode ? 'pointer' : 'not-allowed' }}>Copy barcode</button>
                      <button type="button" onClick={() => openBarcodePreview(item)} disabled={!item.barcode} style={{ padding: '5px 10px', borderRadius: '999px', border: '1px solid #c7d2fe', background: item.barcode ? '#eef2ff' : '#f8fafc', color: item.barcode ? '#4338ca' : '#94a3b8', fontSize: '12px', fontWeight: '700', cursor: item.barcode ? 'pointer' : 'not-allowed' }}>In barcode</button>
                    </div>
                    <div style={{ marginBottom: '10px', padding: '10px 12px', borderRadius: '12px', background: '#f8fafc', border: '1px solid #e2e8f0', color: '#475569', fontSize: '12px', lineHeight: 1.5 }}>
                      <div style={{ fontWeight: '700', color: '#0f172a', marginBottom: '4px' }}>Barcode</div>
                      <div style={{ fontSize: '13px', wordBreak: 'break-all' }}>{item.barcode || 'Chưa có barcode'}</div>
                    </div>
                    {viewFilter !== 'all' && (
                      <div style={{ marginBottom: '10px', padding: '10px 12px', borderRadius: '12px', background: '#f8fafc', border: '1px solid #e2e8f0', color: '#475569', fontSize: '12px', lineHeight: 1.5 }}>
        ...
        <div className="modal-animate" style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000, padding: '16px' }}>
          <div className="modal-panel-animate" style={{ background: 'white', padding: '30px', borderRadius: '16px', width: '100%', maxWidth: '520px', boxShadow: '0 20px 50px rgba(0,0,0,0.18)' }}>
            ...
            <form onSubmit={handleSubmitProduct}>
              ...
              <div style={{ marginBottom: '10px' }}>
                <label style={{ display: 'block', marginBottom: '6px', fontSize: '13px', fontWeight: '600', color: '#475569' }}>Barcode</label>
                <input placeholder="8931234567890" value={newBarcode} onChange={(e) => setNewBarcode(e.target.value)} style={{ width: '100%', padding: '12px 14px', boxSizing: 'border-box', border: '1px solid #dbe3ea', borderRadius: '12px', background: '#fff', outline: 'none' }} />
              </div>
      ...
      {barcodePreviewProduct && (
        <div className="modal-animate" style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.55)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1150, padding: '16px' }}>
          <div className="modal-panel-animate" style={{ background: 'white', padding: '26px', borderRadius: '18px', width: '100%', maxWidth: '420px', boxShadow: '0 20px 50px rgba(0,0,0,0.2)' }}>
            <h3 style={{ marginTop: 0, color: '#1d4ed8' }}>In barcode</h3>
            <div style={{ padding: '18px', border: '1px solid #e2e8f0', borderRadius: '16px', background: '#f8fafc', marginBottom: '16px' }}>
              <div style={{ fontSize: '13px', color: '#64748b', marginBottom: '6px' }}>{barcodePreviewProduct.sku}</div>
              <div style={{ fontSize: '18px', fontWeight: '800', color: '#0f172a', marginBottom: '12px' }}>{barcodePreviewProduct.name}</div>
              <div style={{ fontSize: '14px', letterSpacing: '0.08em', fontFamily: 'monospace', padding: '16px', background: '#fff', border: '1px dashed #cbd5e1', borderRadius: '12px', textAlign: 'center' }}>
                {barcodePreviewProduct.barcode || 'Chưa có barcode'}
              </div>
            </div>
            <div style={{ display: 'flex', gap: '12px' }}>
              <button type="button" onClick={() => copyBarcode(barcodePreviewProduct.barcode)} disabled={!barcodePreviewProduct.barcode} style={{ flex: 1, height: '46px', background: barcodePreviewProduct.barcode ? '#eff6ff' : '#f8fafc', border: '1px solid #dbe3ee', borderRadius: '12px', cursor: barcodePreviewProduct.barcode ? 'pointer' : 'not-allowed', color: barcodePreviewProduct.barcode ? '#1d4ed8' : '#94a3b8', fontWeight: '700' }}>Copy barcode</button>
              <button type="button" onClick={closeBarcodePreview} style={{ flex: 1, height: '46px', background: '#fff', border: '1px solid #e2e8f0', borderRadius: '12px', cursor: 'pointer', color: '#64748b', fontWeight: '700' }}>Đóng</button>
            </div>
          </div>
        </div>
      )}
