...
export default function ReceiptsPage() {
  ...
  const handleBarcodeScan = async () => {
    ...
    const quantity = parseInt(scanQuantity, 10) || 1;
    if (!barcode) return;

    const product = products.find((item) => String(item.barcode || '').trim() === barcode);
    if (!product) {
    ...
    setSelectedItems((prev) => {
      const index = prev.findIndex((item) => String(item.product_id) === String(product.id));
      if (index >= 0) {
        const next = [...prev];
        next[index] = { ...next[index], quantity: Number(next[index].quantity || 0) + quantity };
        return next;
      }
      return [...prev, { product_id: String(product.id), quantity }];
    });
          ...
          <div className="modal-panel-animate" style={{ width: 'min(720px, 100%)', maxHeight: '90vh', overflowY: 'auto', background: '#fff', borderRadius: '24px', border: '1px solid #e5eef8', boxShadow: '0 30px 80px rgba(15, 23, 42, 0.22)' }}>
            ...
            <form onSubmit={handleCreateRequest} style={{ padding: '22px 24px 24px' }}>
              ...
              <div style={{ border: '1px solid #e5eef8', borderRadius: '18px', background: '#f8fbff', padding: '16px', marginBottom: '16px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 2fr) 120px auto', gap: '10px', marginBottom: '12px' }}>
                  <input value={scanBarcode} onChange={(e) => setScanBarcode(e.target.value)} placeholder="Quét / nhập barcode sản phẩm" style={{ padding: '11px 12px', border: '1px solid #dbe3ee', borderRadius: '12px', outline: 'none' }} />
                  <input type="number" min="1" value={scanQuantity} onChange={(e) => setScanQuantity(e.target.value)} style={{ padding: '11px 12px', border: '1px solid #dbe3ee', borderRadius: '12px', outline: 'none' }} />
                  <button type="button" onClick={handleBarcodeScan} style={{ padding: '11px 14px', borderRadius: '12px', border: 'none', background: '#0f172a', color: '#fff', cursor: 'pointer', fontWeight: 700 }}>Thêm bằng barcode</button>
                </div>
