...
const dbPath = process.env.DATABASE_PATH
    ...
    : path.resolve(databaseDir, 'inventory.sqlite');
const schemaPath = path.resolve(databaseDir, 'schema.sql');
const seedPath = path.resolve(databaseDir, 'seed.sql');
...
const runProductColumnMigrations = () => {
    db.all(`PRAGMA table_info(products)`, [], (err, columns) => {
        ...
        if (!columnNames.has('barcode')) {
            pendingStatements.push('ALTER TABLE products ADD COLUMN barcode VARCHAR(100);');
        }
