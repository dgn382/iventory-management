...
const createProduct = (req, res) => {
    const { sku, barcode, name, sale_price, unit, category, image_url, min_stock, warehouse_id, initial_stock } = req.body;

    if (!sku || !name || !sale_price) {
        return res.status(400).json({ message: 'Vui lòng nhập đầy đủ SKU, Tên và Giá' });
    }

    const finalBarcode = barcode && String(barcode).trim() ? String(barcode).trim() : sku;
    const query = `INSERT INTO products (sku, barcode, name, sale_price, unit, category, image_url, min_stock) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;
    
    db.run(query, [sku, finalBarcode, name, sale_price, unit || 'cái', category || null, image_url || null, parseInt(min_stock, 10) || 50], function(err) {
        if (err) {
...
const updateProduct = (req, res) => {
    const { id } = req.params;
    const { sku, barcode, name, sale_price, unit, category, image_url, min_stock, adjust_stock, target_warehouse } = req.body;

    const finalBarcode = barcode && String(barcode).trim() ? String(barcode).trim() : sku;
    const query = `UPDATE products SET sku = ?, barcode = ?, name = ?, sale_price = ?, unit = ?, category = ?, image_url = ?, min_stock = ? WHERE id = ?`;
    db.run(query, [sku, finalBarcode, name, sale_price, unit, category || null, image_url || null, parseInt(min_stock, 10) || 50, id], function(err) {
        if (err) {
            if (err.message.includes('UNIQUE constraint failed')) {
                return res.status(400).json({ message: `Mã SKU "${sku}" đã tồn tại!` });
            }
...
const getProductByBarcode = (req, res) => {
    ...
    db.get(`SELECT * FROM products WHERE barcode = ?`, [barcode], (err, row) => {
        if (err) return res.status(500).json({ message: 'Lỗi Database', error: err.message });
        if (!row) return res.status(404).json({ message: 'Không tìm thấy sản phẩm với barcode này' });
...
module.exports = { getAllProducts, getProductByBarcode, createProduct, updateProduct, deleteProduct };